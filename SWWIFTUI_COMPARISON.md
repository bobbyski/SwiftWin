...
| M2 | `ScrollView` | `ScrollView` / `WinScrollView` | Scoped child HWND movement and visibility clipping | `ScrollViewer` | Win32 now partial, WinUI later | First-pass vertical scroll view exists with wheel scrolling. Native scrollbar thumbs, nested scrolling, robust clipping, and scroll indicators remain planned. |
