...
| M2 | `ColorPicker` | `ColorPicker` / `WinColorPicker` | Custom swatch button now; Color common dialog later | `ColorPicker` | Framework / Win32 now, WinUI later | First pass implemented as an owner-drawn swatch control that cycles through a small built-in palette and supports callbacks/bindings. A native Windows color dialog and richer SwiftUI color model remain planned. |
