...
| M1 | `Button` | `Button` / `WinButton` | `BUTTON` with `BS_OWNERDRAW` | `Button` | Win32 now, WinUI later | Implemented with actions and owner-drawn primary/secondary styles. Pressed, disabled, focused, and explicit hover paint are in progress. |
...
| M2 | `SecureField` | `SecureField` / `WinSecureField` | `EDIT` with password style | `PasswordBox` | Win32 or WinUI | Not implemented. Should reuse text-field binding behavior and add password masking/reveal policy. |
...
| M2 | `ProgressView` | `ProgressView` / `WinProgressView` | Progress bar common control | `ProgressBar`, `ProgressRing` | Win32 or WinUI | Not implemented. Determinate progress maps cleanly; indeterminate ring is better in WinUI or custom drawing. |
| M2 | `Stepper` | `Stepper` / `WinStepper` | Up-down control plus edit/static label | `NumberBox` or custom stepper | Win32 or WinUI | Not implemented. Win32 has an up-down common control, but SwiftUI's value binding and label layout need wrapper logic. |
| M2 | `DatePicker` | `DatePicker` / `WinDatePicker` | Date/time picker common control | `DatePicker`, `TimePicker`, `CalendarDatePicker` | Win32 or WinUI | Not implemented. Native Win32 control exists; SwiftUI style variants and date binding need normalization. |
| M2 | `ColorPicker` | `ColorPicker` / `WinColorPicker` | Color common dialog, custom button | `ColorPicker` | Win32 dialog or WinUI | Not implemented. Win32 has a dialog rather than a full inline SwiftUI-style picker. |
| M2 | `Link` | `Link` / `WinLink` | SysLink or owner-drawn button/text | `HyperlinkButton` | Win32 or WinUI | Not implemented. URL launching and styling should be wrapped safely. |
| M3 | `TextEditor` | `TextEditor` / `WinTextEditor` | Multiline `EDIT` or Rich Edit | Multiline `TextBox`, `RichEditBox` | Win32 or WinUI | Not implemented. Plain editing can begin with multiline `EDIT`; serious editing needs Rich Edit or WebView/Monaco. |
...
| M2 | `Divider` | `Divider` / `WinDivider` | Static line, custom paint, etched frame | `Rectangle`, `Border` | Framework / custom | Not implemented. Custom line drawing is likely better than old etched Win32 styles. |
...
| M3 | `ScrollView` | `ScrollView` / `WinScrollView` | Scrollbars plus child window/content movement | `ScrollViewer` | Win32 now partial, WinUI later | Window-level wheel scrolling exists as a stopgap. Proper `ScrollView` needs contained scrolling, clipping, scrollbars, and nested content. |
...
| M3 | `.foregroundStyle(_:)` | Foreground style modifier | Text color, brush, pen | Foreground brush | Framework + backend | Not implemented. Needs semantic color model and inheritance. |
| M3 | `.background(_:)` | Background modifier | Brush fill/custom paint | Background brush | Framework + backend | Not implemented. Child HWND backgrounds and parent paint order need careful handling. |
| M3 | `.border(_:)` / `.overlay(_:)` | Decoration modifiers | Custom drawing, border styles | Border, overlay composition | Framework + backend | Not implemented. Overlay semantics are easier with retained/custom drawing than immediate child HWND placement. |
