...
| M3 | `.background(_:)` | Background modifier | Brush fill/custom paint | Background brush | Framework + backend | Partial. Solid color backgrounds exist through `WinBackground`; arbitrary background views, materials, clipping, rounded corners, and exact paint order remain planned. |
| M3 | `.border(_:)` / `.overlay(_:)` | Decoration modifiers | Custom drawing, border styles | Border, overlay composition | Framework + backend | Partial. Rectangular `.border(_:width:)` exists through `WinBorder`; overlay, rounded shape strokes, clipping, and exact paint-order semantics remain planned. |
